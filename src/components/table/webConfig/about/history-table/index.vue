  ...
  <div class="history-table">
    ...
    >
      ...
      <el-table-column
        ...
        align="center"
      ></el-table-column>
      <el-table-column prop="time" label="历史年份" align="center">
      </el-table-column>
...
export default {
  ...
  methods: {
    async getTableData(data) {
      const res = await getHistoryTableData(data);
      if (res.code === 200) {
    ...
    async del(row) {
      await this.$confirm(`确定要删除 "${row.time}" 吗?`, "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      });
      const res = await delHistoryTableData(row.id);
      if (res.code === 200) {
