<template>
  <div class="salesman-table">
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="salesPost" label="销售岗位" align="center">
      </el-table-column>
...
import {
  delSalesmanTableData,
  getSalesmanTableData,
...
export default {
  ...
  methods: {
    ...
    async getTableData(data) {
      const res = await getSalesmanTableData(data);
      if (res.code === 200) {
    ...
    // 删除
    async del(row) {
      await this.$confirm(`确定要删除 "${row.salesPost}" 吗?`, "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      });
      const res = await delSalesmanTableData(row.id);
      if (res.code === 200) {
